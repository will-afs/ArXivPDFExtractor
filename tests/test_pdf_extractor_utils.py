...
from src.pdfextractor.core.pdf_extractor_utils import (
                                                    ...
                                                    extract_pdf_content,
                                                    extract_references_from_pdf_content,
                                                    # extract_pdf_raw_data,
                                                    extract_pdf_references,
                                                    #extract_ner_from_pdf_uri,                                                  
)
...
def test_get_file_object_from_uri_wrong_uri_format(mocker):
    with pytest.raises(ValueError):
        ...
        get_file_object_from_uri(WRONG_PDF_URI, COOLDOWN_MANAGER_URI)
...
def test_extract_ner_from_pdf_uri(pdf_bytes, pdf_metadatas_reference, references_reference, mocker):
    mocker.patch('src.pdfextractor.core.pdf_extractor_utils.request_file_through_uri', return_value = pdf_bytes)
    pdf_metadata = extract_pdf_references(pdf_metadatas_reference[0], COOLDOWN_MANAGER_URI)
    
    assert list(pdf_metadata.keys()) == ['uri', 'authors', 'title', 'references'] # 'date',
    assert pdf_metadata['uri'] == pdf_metadatas_reference[0]['uri']
    assert pdf_metadata['authors'] == pdf_metadatas_reference[0]['authors']
    assert pdf_metadata['title'] == pdf_metadatas_reference[0]['title']
